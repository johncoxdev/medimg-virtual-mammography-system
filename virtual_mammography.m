...
grid on;
    ...
    % Calculate slice positions based on tumor locations
    active_tumors = tumor_centers(1:num_tumors, :);  % Get only the active tumor positions
    xpositions = active_tumors(:,1);
    ypositions = active_tumors(:,2);
    zpositions = active_tumors(:,3);
    
    % Create slices that pass through tumor centers and middle of phantom
    xslice = unique([xpositions', size(phantom,1)/2]);
    yslice = unique([ypositions', size(phantom,2)/2]);
    zslice = unique([zpositions', size(phantom,3)/2]);
    
    % Create slice visualization
    h = slice(phantom, xslice, yslice, zslice);
    
    % Set properties for all slices
    for i = 1:length(h)
    ...
    % Set view properties
    view(ax3d, 45, 30);  % Adjusted viewing angle
    grid(ax3d, 'on');
